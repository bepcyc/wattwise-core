...
def _guard_check(prop: str, schema: dict[str, Any], schemas: dict[str, Any]) -> str:
    ...
    primitive = {"string": "string", "integer": "number", "number": "number",
                 "boolean": "boolean"}
    if isinstance(kind, str) and kind in primitive:
        nullable = "null" in (resolved.get("type") or []) if isinstance(
            resolved.get("type"), list) else False
...
def generate(document: dict[str, Any]) -> str:
    ...
    parts = [
        "/* Generated by tools/client_gen.py from /v1/openapi.json — do not edit. */\n"
    ]
